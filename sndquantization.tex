...
We have already seen in Chapter 1 of \cite{lsm} that a string have a collective wave motion that is described by a continuous field, which satisfies the familiar one-dimensional wave equation
\begin{align}
\label{eq:2}
...
Since
\begin{align}
  \sum_{n=-\infty}^{\infty} \frac{1}{2}\to\infty\,,
\end{align}
...
The ground state can be denoted by $|0\rangle$. Since this is state of lowest energy, the annihilation operator $\widehat{a\,}_{n}^\dagger$,
...
\begin{align}
  \widehat{a\,}_{n}^\dagger|0\rangle=&|1_n\rangle\nonumber\\
  \langle0|\widehat{a}_{n}=&\langle1_n|
\end{align}

As $|1_n\rangle$ is an Hamiltonian eigenstate of energy $E_n$, the effect of $\widehat{a\,}_{n}^\dagger$ on the ground state is to create an one-particle eigenstate of energy $E_n$
\begin{align}
...
Noting also that
\begin{align}
  \mathcal{N}_n|m_l\rangle=0\,,
\end{align}
we have that
\begin{align}
  \langle m_n|\colon\!\widehat{H}\colon|m_n\rangle=m_n \omega_n=m_n \hbar\omega_n\,.
\end{align}
Therefore, once we have proper normalized states and renormalized Hamiltonian, the energy of an state with $m$ fields of frequency $\omega_n$ is just $m$ times the energy of the one quanta of energy $\hbar\omega_n$. Note that
\begin{align}
...
The most general Fock state for a system of $N$ simple harmonic oscillators of frequencies $\omega_n$, $n=1,\ldots,N$, with solution
\begin{align}
  \widehat{\phi}=\sum_{n=1}^N\frac{v}{\sqrt{2\omega_nL}}
\left[\widehat{a}_n\,e^{-i(\omega_n t-k_n z)}+\widehat{a\,}_n^\dagger\,e^{i(\omega_n t-k_n z)}\right],
\end{align}

\begin{align}
  |m_1,m_2,\cdots,m_N\rangle=\prod_{n=1}^N\frac{\left(\widehat{a\,}_{n}^\dagger\right)^{m_n}}{\sqrt{m_n !}}|0\rangle\,.
\end{align}
\begin{align}
  |1_1,1_2,\cdots,1_N\rangle=\prod_{n=1}^N{\left(\widehat{a\,}_{n}^\dagger\right)}|0\rangle\,.
\end{align}
...
\begin{align}
 \widehat{\phi}(x)|0\rangle=&\sum_{n=-\infty}^\infty\frac{v}{\sqrt{2\omega_nL}}\widehat{a\,}^\dagger_n 
 e^{i (\omega_n t-k_n z)}|0\rangle\nonumber\\
=&\sum_{n=-\infty}^\infty\frac{v}{\sqrt{2\omega_nL}} e^{i (\omega_n t-k_n z)}|1_n\rangle
\end{align}
On the other hand
\begin{align}
 \langle0| \widehat{\phi}(x')
=&\sum_{m=-\infty}^\infty\frac{v}{\sqrt{2\omega_mL}} e^{-i (\omega_m t'-k_m z')}\langle1_m|
\end{align}
We thus obtain
\begin{align}
  \langle0|\widehat{\phi}(x')\widehat{\phi}(x)|0\rangle
  =&\sum_{n,m=-\infty}^\infty\frac{v^2}{{2L\sqrt{\omega_m\omega_n}}}  
  e^{i [\omega_n t-\omega_m t'-k_n z+k_m z')]}\langle1_m|1_n\rangle
\nonumber\\
  \langle0|\widehat{\phi}(x')\widehat{\phi}(x)|0\rangle
  =&\sum_{n,=-\infty}^\infty\frac{v^2}{{2L\omega_n}}  e^{i [\omega_n (t-t')-k_n (z-z')]}
\end{align}
Note that it is the square of the function, not the function itself.


We can now check eq.~\eqref{eq:1}, From eq.~\eqref{eq:18}

% \begin{align}
%     \widehat{\phi}^+|1_n\rangle=&\sum_{m=-\infty}^\infty\frac{v}{\sqrt{2\omega_mL}}
%   \widehat{a}_m\,e^{-i(\omega_m t-k_m z)}|1_n\rangle\nonumber\\
% =&\sum_{m=-\infty}^\infty\frac{v}{\sqrt{2\omega_mL}}\,e^{-i(\omega_m t-k_m z)}
%   \widehat{a}_m \widehat{a\,}_{n}^\dagger|0\rangle\nonumber\\
% =&\sum_{m=-\infty}^\infty\frac{v}{\sqrt{2\omega_mL}}\,e^{-i(\omega_m t-k_m z)}
%   \left[\widehat{a}_m ,\widehat{a\,}_{n}^\dagger\right]|0\rangle\nonumber\\
% =&\sum_{m=-\infty}^\infty\frac{v}{\sqrt{2\omega_mL}}\,e^{-i(\omega_m t-k_m z)}
%   \delta_{m n}\right]|0\rangle\nonumber\\
% =&\frac{v}{\sqrt{2\omega_nL}}\,e^{-i(\omega_n t-k_n z)}|0\rangle
% \end{align}`
The effect of $\widehat{\phi}^+$ is to annihilate an state with wave function of frequency $\omega_n$
...
\begin{align}
  \sum_{\mathbf{p}} \delta_{\mathbf{p}, \mathbf{q}}=\delta_{\mathbf{p}, \mathbf{p}}=&1
\end{align}
we have
\begin{align}
  \int d^3p \delta^{(3)}(\mathbf{p}-\mathbf{q})=&\sum_n \delta_{nm}\nonumber\\
 \sum_n \left(\frac{2\pi}{L}\right)^3\delta^{(3)}(\mathbf{p}-\mathbf{q})=&\sum_n \delta_{\mathbf{p}, \mathbf{q}}\nonumber\\
  \left(\frac{2\pi}{L}\right)^3\delta^{(3)}(\mathbf{p}-\mathbf{q})=& \delta_{\mathbf{p}, \mathbf{q}}\,.
\end{align}
In this way
\begin{align}
...
\begin{align}
  \label{eq:24}
\left(\frac{L}{2\pi}\right)^3\delta_{\mathbf{p},\mathbf{q}}\to  \delta^{(3)}(\mathbf{p}-\mathbf{q})
\end{align}
...
%    $$&$$&$$\\
    ...
    $\delta_{\textbf{p},\textbf{q}}$&$\left(\frac{2\pi}{L}\right)^3\delta^{(3)}(\mathbf{p}-\mathbf{q})$&
    $\left(\frac{2\pi}{L}\right)^3\delta^{(3)}(\mathbf{p}-\mathbf{q})$\\
...
$|\textbf{p}\rangle=\frac{1}{\sqrt{V}}\widehat{a}^\dagger_{\mathbf{p}}|0\rangle$\\
    $\langle1_{\textbf{n}}|1_{\textbf{m}}\rangle= \delta_{\textbf{n},\textbf{m}}$&$\langle\textbf{p}|\textbf{p}'\rangle=\left(\frac{2\pi}{L}\right)^3 \delta^{(3)}(\mathbf{p}-\mathbf{p}')$&
    $\langle\textbf{p}|\textbf{p}'\rangle=\left(\frac{2\pi}{L}\right)^3 \delta^{(3)}(\mathbf{p}-\mathbf{p}')$\\
  \end{tabular}
  \caption{From discret to continuos}
  \label{tab:666}
\end{table}
...
Another choice of normalization is the Lorentz invariant one, to be used later. In this case, the generic state of the Fock
...
\begin{align}
\label{eq:34}
\langle\mathbf{p}_1 |\mathbf{p}_2\rangle=&
  \left(2E_\mathbf{p_1}\right)^{1/2}\left(2E_\mathbf{p_2}\right)^{1/2}
 \langle0|\widehat{a}_{\mathbf{p}_1} \widehat{a}_{\mathbf{p}_2}^\dagger|0\rangle\nonumber\\
=&\left(2E_\mathbf{p_1}\right)^{1/2}\left(2E_\mathbf{p_2}\right)^{1/2}
 \langle0|\left[\widehat{a}_{\mathbf{p}_1}, 
\widehat{a}_{\mathbf{p}_2}^\dagger\right]|0\rangle\nonumber\\
=&\left(2E_\mathbf{p_1}\right)^{1/2}\left(2E_\mathbf{p_2}\right)^{1/2}\left({2\pi}\right)^3\delta^{(3)}(\mathbf{p}_1-\mathbf{p}_2)\nonumber\\
=&2E_\mathbf{p_1}\left({2\pi}\right)^3\delta^{(3)}(\mathbf{p}_1-\mathbf{p}_2)\,.
\end{align}
...
\begin{align}
  \label{eq:35}
   \langle\mathbf{p}_1 |\mathbf{p}_2\rangle=&2E_\mathbf{p_1}L^3 
...
If we sum over spins we obtain
\begin{align}
  \sum_s u_s(\mathbf{p})\bar{u}_s(\mathbf{p})=\cancel{p}+m\nonumber\\
  \sum_s v_s(\mathbf{p})\bar{v}_s(\mathbf{p})=\cancel{p}-m
\end{align}
For details see \cite{hep-ph/0703214}.
The spinors also satisfy some completeness relations
\begin{align}
...
\end{align}
Even after normal ordering, this Hamiltonian can give negative energy eigenvalues, which is a serious problem. If instead we assume that the creation and annihilation operators satisfy anticommutation relations
\begin{align}
