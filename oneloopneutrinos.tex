...
\begin{align}
  \mathcal{L}=\left(y_{in\alpha}\nu_{i}\chi_n S_{\alpha}+y'_{mi\alpha}\chi_n\nu_{i} S_{\alpha}^{*}+\text{h.c}+m_a \psi_a\psi_a  \right)+m_{\alpha}^2\,S_{\alpha}^2+
\end{align}
...
In the Zee model we can work in the Higgs-basis with $\left\langle H_1 \right\rangle=v/\sqrt{2}$,  $\left\langle H_1 \right\rangle=v/\sqrt{2}=0$ \cite{AristizabalSierra:2006ri}. In that basis the Lagrangian is
