...
\begin{align}
\label{eq:hppp}
  \mathcal{H}_I=\frac{h}{\sqrt{2}}:\overline{\Psi}\Psi\phi:
\end{align}
...
\begin{align}
  S^{(1)}=\nonumber\\
=&-i \frac{h}{\sqrt{2}} \int d^4x:\overline{\Psi}\Psi\phi:\,.
\end{align}

\begin{align}
  S^{(1)}=-i \frac{h}{\sqrt{2}} \int d^4x:(\overline{\Psi}_++\overline{\Psi}_-)(\Psi_++\Psi_-)(\phi_++\phi_-):\,.
\end{align}
...
\begin{align}
  \label{eq:97f}
  -i \frac{h}{\sqrt{2}} \int d^4x\overline{\Psi}_-\Psi_-\phi_+\,.
\end{align}
...
\begin{align}
  S_{fi}^{(1)}=-i \frac{h}{\sqrt{2}} \sum_{s,s'} \int \operatorname{d}^4x &\left[ \left\langle 0, e_L(p),\left( e_R \right)^{\dagger}(p') \left|\xi_-^{\dagger}\eta_-^{\dagger} \phi_{+}  \right|H(\mathbf{k}),0,0 \right\rangle
   \right. +\nonumber\\
...
\end{align}
\begin{align}
  =&\frac{h^2}{2} \left[x^{\alpha}(s',\boldsymbol{p}') x_{\alpha}(s,\boldsymbol{p})+y^{\dagger}_{\dot{\beta}}(s',\boldsymbol{p}')y^{\dagger\dot{\beta}}(s,\boldsymbol{p}) \right]
\left[ x^{\dagger}_{\dot{\alpha}}(s,\boldsymbol{p}){x^{\dagger}}^{\dot{\alpha}}(s',\boldsymbol{p}')+y^{\beta}(s,\boldsymbol{p})y_{\beta}(s',\boldsymbol{p}') \right],
\end{align}
...
Therefore
\begin{align}
   \left| \mathcal{M} \right|^2=4 \left( p\cdot p' - mm' \right)
\end{align}
...
\end{align}
In the case of the standard model Higgs with mass $M_H=M$ decaying to fermion pair, and using $m_f=yv/\sqrt{2}$,  $v=\left( \sqrt{2}G_F \right)^{-1/2}$
...
\end{equation}
For the top quark\footnote{to the top quarks is kinematically forbidden, to the bottom quark a further factor $N_c=3$ must be considered}, taking into account that $1.166 \times 10^{-5}\ \text{GeV}^{-2}$, $M_H=125\ \text{GeV}$, and $m_\tau=1.777\ \text{GeV}$,
\begin{align}
  \Gamma(H\to t\overline{t})=0.26\ \text{MeV}\to \tau= \frac{\hbar}{\Gamma}=2.53\times 10^{-21}\ \text{seconds}\,.
\end{align}
