...
\end{align}
In the non-relativistic limit we have $t\approx 0$
\begin{align}
    \frac{d\sigma}{d\Omega}
=&\frac{y^4}{128\pi^2 \left(m_1+m_2\right)^2}
  \frac{ \left(2m_1^2  \right)\left( 2m_2^2 \right)}{\left[m^2_h -\left(m_1-m_2  \right)^2\right]^2} \nonumber\\
=&\frac{y^4}{32\pi^2}\left(\frac{m_1m_2}{m_1+m_2}  \right)^2\frac{1}{\left(m_h^2-m_1^2-m_2^2+2m_1m_2  \right)^2}
\end{align}
...
D_{\mu\nu} \left( x_1-x_2 \right)\nonumber\\
&\left\langle q_L \left(\mathbf{p}'_2\right),e_L\left(\mathbf{p}'_1\right)\right|
   \xi^{\dagger}_{\dot{\alpha} -}(x_1)\xi^{\dagger}_{\dot{\beta}-}(x_2)\left( \overline{\sigma}^{\mu} \right)^{\dot{\alpha}\alpha}\left( \overline{\sigma}^{\nu} \right)^{\dot{\beta}\beta}\xi_{\alpha +}(x_1)\xi_{\beta +}(x_2)
 \left| q_L\left(\mathbf{p}_2\right), e_L \left(\mathbf{p}_1\right)\right\rangle. 
\end{align}
...
\subsubsection{Scalar mediated Lepton quark scattering}

mediated by $h$, with couplings
\begin{align}
\label{eq:xietaz}
...
and
\begin{align}
  \sum_{\text{spins}}y'_{1\alpha}y'_{2\beta}x_1^{\alpha}x_2^{\beta} x_2^{\dagger\dot{\beta}} x_1^{\dagger\dot{\alpha}} y^{\prime\dagger}_{2\dot{\beta}} y^{\prime\dagger}_{1\dot{\alpha}}
\end{align}
